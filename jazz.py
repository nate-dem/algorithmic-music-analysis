...
def key_distribution(df, keys):
    ...
    # graph structure 
    ax.set_title("Key Distribution in Jazz")
...
def major_minor_keys(df, major_keys, minor_keys):
    ...
    sns.countplot(data=df, x="Mode", palette=custom_palette, order=["Minor", "Major"])
    plt.title("Major vs. Minor Keys in Jazz", fontsize=16)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("jazz_keys_analysis.csv")
